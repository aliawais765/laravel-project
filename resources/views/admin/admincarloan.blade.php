...
<div class="container">
  <div class="profile-table">
    <h3 class="mb-4 text-center">car-loan</h3>
    @if (session('success'))
    <div class="alert alert-success" role="alert" style="background-color:      background-color: #343a40;
">
        {{ session('success') }}
    </div>
  @endif
    <table class="table table-hover align-middle text-center">
      ...
      <td >{{ $bankloan->notes }} <td><a href="{{route('editcarloan',$bankloan->id)}}"><button class="btn btn-edit btn-sm me-2">Edit</button></a></td><td><form action="{{ route('deletecarloan', $bankloan->id) }}" method="POST">
    @csrf
    @method('DELETE')
